...
def loss(w, x, y):
    ...
    res = np.dot(diff, diff) / y.shape[0] + gamma * np.square(np.linalg.norm(w))
    return res

def grad(w, x, y):
    res = (- 2 * np.dot(x.T, (y - np.dot(x, w))))/ y.shape[0] + 2*gamma*w
    return res

def one_func_loss(w, x, y, i):
    my_y = np.dot(x[i], w)
    diff = (y[i] - my_y) #.reshape(my_y.shape[0],)
    res = np.dot(diff, diff) + gamma * np.square(np.linalg.norm(w))
    return res

def one_func_grad(w, x, y, i):
    x_loc = x[i]
    my_y = np.dot(x_loc, w)
    res = ((-2 * (y[i] - my_y)) * x_loc).reshape(w.shape) + 2 * gamma * w
    return res

def batch_grad(w, x, y, i0, i1):
    x_loc = x[i0:i1]
    y_loc = y[i0:i1]
    res = (- 2 * np.dot(x_loc.T, (y_loc - np.dot(x_loc, w)))) / y_loc.shape[0] + 2 * gamma * w
    return res
...
def batch_loss(w, x, y, i0, i1):
    ...
    res = np.dot(diff, diff) / y_loc.shape[0] + gamma * np.square(np.linalg.norm(w))
    return res
...
opt_res = op.minimize(fun=lambda w: loss(w.reshape((w.size, 1)), x, y), x0=w0, tol=1e-13)
