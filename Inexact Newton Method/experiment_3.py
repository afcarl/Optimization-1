...
from optim import newton, plot_performance
...
dim = 4  # D - 1
data_size = 1000  # N
mu, sigma1 = 0, 1 # Parameters for generating the data and the starting point
x_interval = 1  # Data scaling parameter
sparse = False  # Boolean, indicating weather or not we use sparse data
...
optimal_value = (minimize(fun, w0.reshape((w.shape[0], 1)), method="Newton-CG",
                          options={'disp': False, 'maxiter': 100}, tol=1e-5, jac=grad))['fun']

print("Optimal Value Found")
# Newton method
w_opt_newton, hist_newton = newton(oracle_hess, w0, disp=True, maxiter=200, tol=1e-7)
